...
#          Ef1   ,   Ef2    ,    Eb2    ,     Ef3  ,      Eb3   ,    Ef4    ,   Eb4     ,    Ef5   ,    Eb5     ,     Ef_5  ,   Eb_5   ,   Ef6  ,     Eb6    ,   Ef_6    ,   Eb_6   ,    Ef7   ,     Ef8  
# Eng = (72512.78, 125540.90, 113172.2678, 71470.35, 96038.39527, 136039.66, 100315.8504, 167340.88, 139748.1857,  0.0000000   , 0.000000 , 15584.5, 67225.0236,  0.00000  , 0.00000  , 38836.92, 105539.25)          
...
t_end = 300 # Time end
th_NH3_0 = ( 0.45, )#0.35,     0.25, 0.15, 0.05)
theta = np.ones(len(th_NH3_0)) -th_NH3_0
...
def dydt(t,y, Ef1, Ef2, Eb2, Ef3, Eb3, Ef4, Eb4, Ef5, Eb5, Ef_5, Eb_5, Ef6, Eb6, Ef7, Ef8):
    ...
    Th_NH3, Th_NH2, Th_NH, Th_N, Th_NN, Th_NNH, Th_N2, Th_H,  Th,  P_N2,  P_H2 = y 
        
    # calculate all reaction rate constants        
        #Dissociation of NH3
    k_ads_1 = 0 # adsorption of NH3
    k_des_1 = 0 # desorption of NH3 
    ...
    k_f5 = k_surf(T, Ef_5)/Beta * Qvib(T, frq_TS_5) /Qvib(T, frq_IS5) #!!! + alpha_N*  (Th_N+Th_NH +Th_H -0.33)  *Exp(Th_N)
    ...
    k_b_8 = 0# H2 dissociative adsorbtion
...
# Rotational constant B source: http://www.lifesci.sussex.ac.uk/research/fluorine/p5qsp3l/sw_teaching/f1177_html/rotlab/node15.html
    ...
    rf1 = 0 #k_ads_1 * P_NH3* Th # adsorption and desorption of NH3
    rb1 = 0 #k_des_1 * Th_NH3  
    r1 = rf1 - rb1
    ...
    rf_6 = k_f6 * Th_NNH  #  *N-NH* <=> N2* + H
    rb_6 = k_b6 * Th_N2 * Th_H ;  
    r_6 = rf_6 - rb_6
...
for k in range(len(th_NH3_0)):
    y0 = [th_NH3_0[k] , 0 , 0 , 0 , 0 , 0 , 0 , 0, theta[k] , 0 , 0]
    r = solve_ivp(dydt, (0,t_end), y0, method="BDF", t_eval=np.linspace(0,t_end,3000), rtol = 1e-12, atol=1e-15, args =arguments)

    Th_NH3, Th_NH2, Th_NH, Th_N, Th_NN, Th_NNH, Th_N2, Th_H,  Th,  P_N2,  P_H2 = r.y 
    ...
    #  4/ Caculate Rate
    # =============================================================================
    print("Starting rate calculations round #"+ str(k+1))

    #------------------------------------
# fig, ax = plt.subplots(figsize=(8, 7))
    Identity = np.ones(3000)
    for i in range(len(T)):
        k7 = k_surf(T,Ef7 )/Beta *Qrot(T,2,1.9982)*Qvib(T[i],frq_FS7)/Qvib(T[i],frq_IS7) *Qtran(T,m_N2)
        r7 = k7*Th_N2        
    #------------------------------------    
    # H2 
        k_f_8 = k_surf(T,Ef8 + alpha_N* (Th_N[i]-0.33*Identity[i])*Exp(Th_N[i]))/Beta  *Qrot(T,2,60.853)*Qvib(T[i],frq_FS8)/Qvib(T[i],frq_IS8) *Qtran(T,m_H2) # H-H coupling
        r8 = k_f_8 * Th_H**2       
    print("round #"+ str(k+1) +" done")
    ...
    # # data[''] = r5
    # data['H2'] = r8
    # ascii.write(data, 'Ni_TPD_Cov_'+str(th_NH3_0[k])+'.dat', overwrite=True)    
        
        
    # =============================================================================
    # Plot figures
    # =============================================================================
    print("plot TPD at initital coverage = "+str(round(th_NH3_0[k],2)))
    
    # ----------------------------------TPD of N2------------------------------------
    plt.plot(T,r7, label= str(round(th_NH3_0[k],2)) +"ML", color ="r", ls = str(linestyle[k]))
    plt.title(r"TPD of $N_2$ at $\beta$= " + B +r" K/s on $Ni$ at different coverage", fontsize=18)
    
    # ----------------------------------TPD of H2------------------------------------
    plt.plot(T,r8, label= str(round(th_NH3_0[k],2)) +"ML", color ="r", ls = str(linestyle[k]))
    plt.title(r"TPD of $H_2$ at $\beta$= " + B +r" K/s on $Ni$ at different coverage", fontsize=18)
